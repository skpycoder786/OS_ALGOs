...
import pandas as pd
id=[]
N=int(input("Enter no. of processes in LJF Sheduling :"))
for i in range(N):
    id.append(i)
...
# Making of Gantt Chart (LJF)    ****GAME_CHANGES_HERE****
print("\nGantt Chart for processes in LJF Sheduling ...\n")
id_at = [(k,v) for k,v in df["AT"].items()] #list of tuple(id,at)
sort_tup(id_at) # sorting by AT values
t = id_at[0][1]
print(t,end="   ")
print("P[%d]"%(id_at[0][0]),end="   ")
t += df["BT"][id_at[0][0]]
print(t,end="   ")
df["CT"][id_at[0][0]]=t
id_at.remove(id_at[0])
while len(id_at) != 0 :
    temp=[]
    for k,v in id_at:
        if v <= t :
            temp.append((k,df["BT"][k]))
    try:        
        p=max(temp, key = lambda x: x[1])[0]
    except:
        val=id_at[0][1]     # special case
        if t < val :      
            delay = val-(t)
